...
%        readyplayers = [],
%        board = gremlin_game:initialize_board(2, 2, PlayerList)
%    },
%    {ok, prepare_phase, M}.
%
%callback_mode() ->
...
%                ),
%                TimerMS = 20000,
%                gremlin_protocol:match_state_update(
%                    pack_tiles(B),
%                    [],
%                    'DECIDE',
%                    PlayerList,
...
%                ),
%                [
%                    gremlin_protocol:player_state_update(
%                        gremlin_db:player_shadow(X),
%                        ID
%                    )
%                    || X <- PlayerList
%                ],
...
%        Data,
%    gremlin_game:maybe_notify_intent(PlayerActions, ID),
%    Ready = [Player | RP],
...
%    % transaction atomic.
%    PlayerShadows = [gremlin_db:player_shadow(X) || X <- P],
...
%    {A1, M1, P1, B1, Replay} = gremlin_game:calculate_round(
...
%    [
%        gremlin_protocol:player_state_update(gremlin_db:player_shadow(X), ID)
%        || X <- P
%    ],
...
%    gremlin_protocol:match_state_update([], [], 'FINISH', P, [], 0, ID),
%    {stop, normal, Data}.
%
%pack_tiles(Board) ->
%    [
