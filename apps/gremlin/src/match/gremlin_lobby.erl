...
%            Result -> Result#gremlin_match.players
%        end,
%    {reply, Reply, {NextID, Matches, Timer}};
%handle_call(
%    {create_match, Player, Mode, MaxPlayers, Extra},
...
%    UpdatedMatches =
%        case UpdatedMatch#gremlin_match.players of
%            [] ->
...
%                gremlin_instance:finalize(MatchID),
...
%    Creating = [X || X <- Matches, X#gremlin_match.state == 'CREATING'],
%    % Then filter for all matches that are older than 1 hr
%    Stale = [
%        X
%        || X <- Creating, CurrentTime - X#gremlin_match.start_time > 3600
%    ],
...
%        X
%        || X <- M1,
%           length(X#gremlin_match.players) =:= 0,
%           CurrentTime - X#gremlin_match.start_time > 300
%    ],
%    M1 -- Empty.
%
%match_find(MatchID, Matches) ->
%    lists:keyfind(MatchID, #gremlin_match.id, Matches).
%
%match_get(false) ->
%    {error, no_such_match};
%match_get(Match) ->
...
%    Matches;
%match_del(Match, Matches) ->
%    MatchID = Match#gremlin_match.id,
%    lists:keydelete(MatchID, #gremlin_match.id, Matches).
%
...
%    if
%        Match#gremlin_match.state =:= 'PLAYING' ->
%            gremlin_instance:remove_player(Player, Match#gremlin_match.id);
%        true ->
%            ok
%    end,
%    Players = Match#gremlin_match.players,
%    UpdatedPlayers = lists:delete(Player, Players),
%    UpdatedMatch = Match#gremlin_match{players = UpdatedPlayers},
%    {ok, UpdatedMatch}.
%
%maybe_join(_Player, false) ->
%    {{error, no_such_match}, false};
%maybe_join(_Player, Match) when
%    length(Match#gremlin_match.players) >= Match#gremlin_match.players_max
%->
...
%        ok ->
%            Players = Match#gremlin_match.players,
%            {ok, Match#gremlin_match{players = [Player | Players]}};
%        Error ->
%            {Error, Match}
%    end.
%
%is_unstarted_match(M) when M#gremlin_match.state == 'CREATING' ->
%    ok;
%is_unstarted_match(_M) ->
%    {error, already_started}.
%
%is_valid_player(Player) ->
%    case gremlin_db:is_valid_player(Player) of
%        true -> ok;
%        false -> {error, invalid_player}
%    end.
%
%is_not_in_match(Player, Match) ->
%    case lists:member(Player, Match#gremlin_match.players) of
%        false -> ok;
...
%maybe_start(Match, Matches) when length(Match#gremlin_match.players) == 0 ->
%    {{error, empty_match}, Matches};
%maybe_start(Match, Matches) ->
%    supervisor:start_child(gremlin_instance_sup, [
%        Match#gremlin_match.players,
%        Match#gremlin_match.id
%    ]),
%    UpdatedMatch = Match#gremlin_match{state = 'PLAYING'},
%    UpdatedMatches = match_update(UpdatedMatch, Match, Matches),
%    {ok, UpdatedMatches}.
%
%repack_match(Match) ->
%    {
%        Match#gremlin_match.id,
%        Match#gremlin_match.state,
%        Match#gremlin_match.players,
%        Match#gremlin_match.players_max,
%        Match#gremlin_match.start_time,
%        Match#gremlin_match.mode,
%        Match#gremlin_match.extra
%    }.
%
%%%===================================================================
%%% Unit Tests
...
%    Matches = gremlin_lobby:get_matches(),
%    ?assertEqual([], Matches).
%
%add_match_test() ->
...
%    _Resp0 = gremlin_db:create_account(Email, "test"),
%    _Resp1 = gremlin_game:new_player(
%        Name,
...
%    ?assertEqual(ok, gremlin_db:delete_player(Name, Email)),
%    ?assertEqual(ok, gremlin_db:delete_account(Email)).
%
%start_match_test() ->
%    % Create a player
%    Name = "Chester Tester",
