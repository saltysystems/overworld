...
%]).
%
%-include("net/gremlin_session.hrl").
...
%    {Msg, State1} =
%        case gremlin_lobby:create_match(Player, Mode, MaxPlayers, Extra) of
%            {ok, MatchID} ->
%                {ok, M} = gremlin_lobby:get_match(MatchID),
...
%    Msg =
%        case gremlin_lobby:join_match(Player, MatchID) of
%            {ok, M} ->
...
%        role = 'DESTROYER'
%    }),
%    {[RespOp2, _RespMsg2], _State} = gremlin_player:new(
%        Message2,
%        State
%    ),
...
%        role = 'DESTROYER'
%    }),
%    {[RespOp3, _RespMsg3], _} = gremlin_player:new(
%        Message3,
%        State
%    ),
...
%    Msg =
%        case gremlin_lobby:leave_match(Player, MatchID) of
%            ok ->
%                Resp = #'ResponseObject'{status = 'OK'},
%                deregister_session(MatchID),
...
%    Msg =
%        case gremlin_lobby:start_match(MatchID) of
%            ok ->
%                Resp = #'ResponseObject'{status = 'OK'},
...
%    case gremlin_lobby:get_match(MatchID) of
%        {ok, M} ->
%            Resp = #'ResponseObject'{status = 'OK'},
...
%    gremlin_instance:player_ready(Player, MatchID),
%    {ok, State};
...
%    Inv = gremlin_db:player_inventory(Player),
%    Email = State#session.email,
...
%                fun() -> gremlin_game:check_player_alive(Player) end,
%                fun() -> gremlin_game:check_valid_items(Actions, Inv) end,
%                fun() ->
%                    gremlin_game:check_valid_target(Actions, MatchID)
%                end])
%        of
%            ok -> true;
%            Error -> Error
...
%    gremlin_instance:player_decision(Player, Actions, MatchID),
%    {ok, State};
...
%    case gremlin_lobby:get_match_players(MatchID) of
%        [H | _T] ->
%            case H =:= Player of
%                true -> ok;
...
%    case gremlin_db:is_valid_player_account(Player, Email) of
%        true -> ok;
...
%%    Players = gremlin_lobby:get_match_players(MatchID),
