...
generate_signals([], _SignalSeen, St0) ->
    St0;
generate_signals([{_OpCode, {{_Module, _Fun, _Arity}, {_ProtoLib, none}}} | Rest], SignalsSeen, St0) ->
    generate_signals(Rest, SignalsSeen, St0);
generate_signals([{_OpCode, {{_Module, _Fun, _Arity}, {ProtoLib, ProtoMsg}}} | Rest], SignalsSeen, St0) ->
    case lists:member(ProtoMsg, SignalsSeen) of
        false ->
            Fields = "(" ++ fields_to_str(field_names({ProtoLib, ProtoMsg})) ++ ")",
...
generate_opcodes([], [], St0) ->
	St0 ++ "}~n~n";
generate_opcodes([{OpCode, {{_M, F, _A}, {_ProtoLib, _ProtoMsg}}} | R], [], St0) ->
	OpString = io_lib:format("~p", [OpCode]),
	Op = ?TAB ++ string:to_upper(atom_to_list(F)) 
         ++ " = " ++ "bytepack(" ++ OpString ++ "),~n",
...
generate_router([], Routes, St0) -> 
    St0 ++ Routes;
generate_router([{_OpCode, {{_Module, Fun, _Arity}, {_ProtoLib, _ProtoMsg}}} | Rest], Routes, St0) -> 
	Op = 
	?TAB ++ ?TAB ++ "OpCode." ++ string:to_upper(atom_to_list(Fun)) ++ ":~n" ++
...
generate_unmarshall([], St0) -> 
	St0;
generate_unmarshall([{_Opcode, {{_Module, Fun, _Arity}, {_ProtoLib, none}}} | Rest], St0) ->
	Op = 
	?TAB ++ "func " ++ atom_to_list(Fun) ++ "(packet):~n" ++
	?TAB ++ ?TAB ++ "print(\"[INFO] Received a " ++ atom_to_list(Fun) ++ " packet (no-op)\")~n~n",
	generate_unmarshall(Rest, St0 ++ Op);
generate_unmarshall([{_Opcode, {{_Module, Fun, _Arity}, {ProtoLib, ProtoMsg}}} | Rest], St0) ->
	FunStr = atom_to_list(Fun),
	Op = 
	?TAB ++ "func " ++ FunStr ++ "(packet):~n" ++
	?TAB ++ ?TAB ++ "print(\"[INFO] Processing a " ++ FunStr ++ " packet\")~n" ++
	?TAB ++ ?TAB ++ "var m = Proto." ++ atom_to_list(ProtoMsg) ++ ".new()~n" ++
	?TAB ++ ?TAB ++ "if result_code != Proto.PB_ERR.NO_ERRORS:~n" ++
	?TAB ++ ?TAB ++ ?TAB ++ "print(\"[CRITICAL] Error decoding new " ++ FunStr ++ " packet\")~n" ++
	?TAB ++ ?TAB ++ ?TAB ++ "return~n",
	Vars = unmarshall_var({ProtoLib, ProtoMsg}),
	Signal = 
	?TAB ++ ?TAB ++ "emit_signal(\"" ++ FunStr ++ "\"," ++ fields_to_str(field_names({ProtoLib, ProtoMsg})) ++ "\)~n~n",
...
unmarshall_var([], Acc) -> Acc;
unmarshall_var([H|T], Acc) ->
	V = ?TAB ++ ?TAB ++ "var " ++ atom_to_list(H) ++ " = " ++ "m.get_" ++ atom_to_list(H) ++ "()~n",
