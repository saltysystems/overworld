...
-define(DEFAULT_ENCODER, saline_pb).

write() ->
    file:write_file(
        "apps/saline_core/priv/static/libsaline.gd", saline_binding:print()
    ).

print() ->
    Ops = [
        saline_protocol:op_info(X)
     || X <- saline_protocol:registered_ops()
    ],
...
load_scripts([H | T], Seen, Acc) ->
    case saline_rpc:encoder(H) of
        undefined ->
            % do we need to do anthing for the default case?  e.g., saline_pb
...
generate_signals([OpInfo | Rest], SignalsSeen0, St0) ->
    % Server ---> Client
    Encoder = saline_rpc:encoder(OpInfo),
    MsgFromServer = saline_rpc:s2c_call(OpInfo),
...
next_opcode([OpInfo | Rest], ok, St0) ->
    OpCode = saline_rpc:opcode(OpInfo),
...
generate_unmarshall([OpInfo | Rest], St0) ->
    ServerMsg = saline_rpc:s2c_call(OpInfo),
    Encoder = correct_encoder(saline_rpc:encoder(OpInfo), ServerMsg),
...
generate_marshall(
    ...
    case saline_rpc:c2s_handler(OpInfo) of
        undefined ->
            % No message to pack
...
opcode_name_string(OpInfo) ->
    ...
    case saline_rpc:c2s_handler(OpInfo) of
        ...
        undefined ->
            ...
            case saline_rpc:s2c_call(OpInfo) of
