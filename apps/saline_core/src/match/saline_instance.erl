...
%        readyplayers = [],
%        board = saline_game:initialize_board(2, 2, PlayerList)
%    },
%    {ok, prepare_phase, M}.
%
%callback_mode() ->
...
%                ),
%                TimerMS = 20000,
%                saline_protocol:match_state_update(
%                    pack_tiles(B),
%                    [],
%                    'DECIDE',
%                    PlayerList,
...
%                ),
%                [
%                    saline_protocol:player_state_update(
%                        saline_db:player_shadow(X),
%                        ID
%                    )
%                    || X <- PlayerList
%                ],
...
%        Data,
%    saline_game:maybe_notify_intent(PlayerActions, ID),
%    Ready = [Player | RP],
...
%    % transaction atomic.
%    PlayerShadows = [saline_db:player_shadow(X) || X <- P],
...
%    {A1, M1, P1, B1, Replay} = saline_game:calculate_round(
...
%    [
%        saline_protocol:player_state_update(saline_db:player_shadow(X), ID)
%        || X <- P
%    ],
...
%    saline_protocol:match_state_update([], [], 'FINISH', P, [], 0, ID),
%    {stop, normal, Data}.
%
%pack_tiles(Board) ->
%    [
