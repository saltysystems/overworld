...
%            Result -> Result#saline_match.players
%        end,
%    {reply, Reply, {NextID, Matches, Timer}};
%handle_call(
%    {create_match, Player, Mode, MaxPlayers, Extra},
...
%    UpdatedMatches =
%        case UpdatedMatch#saline_match.players of
%            [] ->
...
%                saline_instance:finalize(MatchID),
...
%    Creating = [X || X <- Matches, X#saline_match.state == 'CREATING'],
%    % Then filter for all matches that are older than 1 hr
%    Stale = [
%        X
%        || X <- Creating, CurrentTime - X#saline_match.start_time > 3600
%    ],
...
%        X
%        || X <- M1,
%           length(X#saline_match.players) =:= 0,
%           CurrentTime - X#saline_match.start_time > 300
%    ],
%    M1 -- Empty.
%
%match_find(MatchID, Matches) ->
%    lists:keyfind(MatchID, #saline_match.id, Matches).
%
%match_get(false) ->
%    {error, no_such_match};
%match_get(Match) ->
...
%    Matches;
%match_del(Match, Matches) ->
%    MatchID = Match#saline_match.id,
%    lists:keydelete(MatchID, #saline_match.id, Matches).
%
...
%    if
%        Match#saline_match.state =:= 'PLAYING' ->
%            saline_instance:remove_player(Player, Match#saline_match.id);
%        true ->
%            ok
%    end,
%    Players = Match#saline_match.players,
%    UpdatedPlayers = lists:delete(Player, Players),
%    UpdatedMatch = Match#saline_match{players = UpdatedPlayers},
%    {ok, UpdatedMatch}.
%
%maybe_join(_Player, false) ->
%    {{error, no_such_match}, false};
%maybe_join(_Player, Match) when
%    length(Match#saline_match.players) >= Match#saline_match.players_max
%->
...
%        ok ->
%            Players = Match#saline_match.players,
%            {ok, Match#saline_match{players = [Player | Players]}};
%        Error ->
%            {Error, Match}
%    end.
%
%is_unstarted_match(M) when M#saline_match.state == 'CREATING' ->
%    ok;
%is_unstarted_match(_M) ->
%    {error, already_started}.
%
%is_valid_player(Player) ->
%    case saline_db:is_valid_player(Player) of
%        true -> ok;
%        false -> {error, invalid_player}
%    end.
%
%is_not_in_match(Player, Match) ->
%    case lists:member(Player, Match#saline_match.players) of
%        false -> ok;
...
%maybe_start(Match, Matches) when length(Match#saline_match.players) == 0 ->
%    {{error, empty_match}, Matches};
%maybe_start(Match, Matches) ->
%    supervisor:start_child(saline_instance_sup, [
%        Match#saline_match.players,
%        Match#saline_match.id
%    ]),
%    UpdatedMatch = Match#saline_match{state = 'PLAYING'},
%    UpdatedMatches = match_update(UpdatedMatch, Match, Matches),
%    {ok, UpdatedMatches}.
%
%repack_match(Match) ->
%    {
%        Match#saline_match.id,
%        Match#saline_match.state,
%        Match#saline_match.players,
%        Match#saline_match.players_max,
%        Match#saline_match.start_time,
%        Match#saline_match.mode,
%        Match#saline_match.extra
%    }.
%
%%%===================================================================
%%% Unit Tests
...
%    Matches = saline_lobby:get_matches(),
%    ?assertEqual([], Matches).
%
%add_match_test() ->
...
%    _Resp0 = saline_db:create_account(Email, "test"),
%    _Resp1 = saline_game:new_player(
%        Name,
...
%    ?assertEqual(ok, saline_db:delete_player(Name, Email)),
%    ?assertEqual(ok, saline_db:delete_account(Email)).
%
%start_match_test() ->
%    % Create a player
%    Name = "Chester Tester",
