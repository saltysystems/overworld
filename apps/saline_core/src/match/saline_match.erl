...
%]).
%
%-include("net/saline_session.hrl").
...
%    {Msg, State1} =
%        case saline_lobby:create_match(Player, Mode, MaxPlayers, Extra) of
%            {ok, MatchID} ->
%                {ok, M} = saline_lobby:get_match(MatchID),
...
%    Msg =
%        case saline_lobby:join_match(Player, MatchID) of
%            {ok, M} ->
...
%        role = 'DESTROYER'
%    }),
%    {[RespOp2, _RespMsg2], _State} = saline_player:new(
%        Message2,
%        State
%    ),
...
%        role = 'DESTROYER'
%    }),
%    {[RespOp3, _RespMsg3], _} = saline_player:new(
%        Message3,
%        State
%    ),
...
%    Msg =
%        case saline_lobby:leave_match(Player, MatchID) of
%            ok ->
%                Resp = #'ResponseObject'{status = 'OK'},
%                deregister_session(MatchID),
...
%    Msg =
%        case saline_lobby:start_match(MatchID) of
%            ok ->
%                Resp = #'ResponseObject'{status = 'OK'},
...
%    case saline_lobby:get_match(MatchID) of
%        {ok, M} ->
%            Resp = #'ResponseObject'{status = 'OK'},
...
%    saline_instance:player_ready(Player, MatchID),
%    {ok, State};
...
%    Inv = saline_db:player_inventory(Player),
%    Email = State#session.email,
...
%                fun() -> saline_game:check_player_alive(Player) end,
%                fun() -> saline_game:check_valid_items(Actions, Inv) end,
%                fun() ->
%                    saline_game:check_valid_target(Actions, MatchID)
%                end])
%        of
%            ok -> true;
%            Error -> Error
...
%    saline_instance:player_decision(Player, Actions, MatchID),
%    {ok, State};
...
%    case saline_lobby:get_match_players(MatchID) of
%        [H | _T] ->
%            case H =:= Player of
%                true -> ok;
...
%    case saline_db:is_valid_player_account(Player, Email) of
%        true -> ok;
...
%%    Players = saline_lobby:get_match_players(MatchID),
