...
%%
%%=========================================================================
-module(saline_session).

-behaviour(saline_rpc).

% Required saline callbacks
-export([rpc_info/0]).
...
-define(RPC(OpCode, Callback, Arity, ProtoMessage),
    {OpCode, {{?MODULE, Callback, Arity}, {saline_pb, ProtoMessage}}}
).
...
session_ping(Msg, Session) ->
    D = saline_pb:decode_msg(Msg, session_ping),
    ID = maps:get(id, D),
    Last = saline_beacon:get_by_id(ID),
    Now = erlang:monotonic_time(),
    Latency = erlang:convert_time_unit(
        round((Now - Last) / 2), native, millisecond
    ),
    Session1 = set_latency(Latency, Session),
    Resp = saline_pb:encode_msg(#{latency => Latency}, session_pong),
...
session_id_req(_Msg, Session) ->
    ...
    Resp = saline_pb:encode_msg(#{id => ID}, session_id),
...
encode_log_test() ->
    ...
    Decoded = saline_pb:decode_msg(Message, session_log),
