...
run(N, Count, Area, Depth, Test) ->
    F = fun(_Elem, AccIn) ->
        ow_ecs:start(microbench),
        R = [timer(Count, Area, Depth, Test, microbench) | AccIn],
        ow_ecs:stop(microbench),
        R
    end,
...
generate_random_entities(Number, XRange, YRange, World) ->
    ...
    ow_ecs:match_components([phys, hitbox], World).
...
check_area_random_test(QuadTree, Entities) ->
    ...
    [H | _T] = Entities,
    {_EID, EComponents} = H,
    #{pos := EPos} = ow_ecs:get(phys, EComponents),
    ...
    BoundingBox = fun({_ID, Components}) ->
        ...
        Phys = ow_ecs:get(phys, Components),
        #{pos := Pos} = Phys,
        Hitbox = ow_ecs:get(hitbox, Components),
